    ...
    async run(client, message, args, prefix, player) {
        try {
            ...
            if (!db || db === false || db === undefined) {
                player.send({ op: 'filters', guildId: message.guild.id, equalizer: [{ band: 0, gain: 0.10 },{ band: 1, gain: 0.10 },{ band: 2, gain: 0.05 },{ band: 3, gain: 0.05 },{ band: 4, gain: -0.05 },{ band: 5, gain: -0.05 },{ band: 6, gain: 0 },{ band: 7, gain: -0.05 },{ band: 8, gain: -0.05 },{ band: 9, gain: 0 },{ band: 10, gain: 0.05 },{ band: 11, gain: 0.05 },{ band: 12, gain: 0.10 },{ band: 13, gain: 0.10 }] });
                player.data.set(`bass`, true);
                return send(`**| Enabled BassBoost**`);
            }
            if (db === true) {
                player.send({ op: 'filters', guildId: message.guild.id });
